...
function applyHelper (aggregate, aggregateModel, cmd) {
  return function (name, payload, version) {
    ...
    dotty.put(evt, aggregate.definitions.event.causationId, dotty.get(cmd, aggregate.definitions.command.id));
    dotty.put(evt, aggregate.definitions.event.correlationId, 
      dotty.get(cmd, aggregate.definitions.command.correlationId) ||
      dotty.get(cmd, aggregate.definitions.command.id)
    );
